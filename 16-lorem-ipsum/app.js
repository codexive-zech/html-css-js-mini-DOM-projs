...
form.addEventListener('submit', function (e) {
  // prevent the form default behavior of submitting to the server
  e.preventDefault();
  //   Getting the Value of the Input and casting it to Number
  const inputValue = parseInt(amount.value);

  const randomText = getRandomText();
  //   checking to see if the Input value is either EMPTY, LESS THAN 0 or GREATER THAN 9
  if (isNaN(inputValue) || inputValue <= 0 || inputValue > 9) {
    //   dynamically displaying the value from the text array
    result.innerHTML = `<p class="result">${text[randomText]}</p>`;
  } else {
    //   getting copy's of the beginning to the end values in the array
    let tempText = text.slice(0, inputValue);
    // mapping out the value of the start to end in the array
    tempText = tempText.map(function (item) {
      return `<p class="result">${item}</p>`;
    });
    //   dynamically displaying the value from 0 to inputValue in HTML from the text array
    result.innerHTML = tempText;
  }
});

// getting the random Number via a function
